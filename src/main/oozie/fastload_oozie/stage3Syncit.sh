...
if [ "$#" -ne 2 ]; then
	...
	rawJobID=$(oozie job -oozie http://10.1.132.24:11000/oozie -config "/hdfs_mount/user/${uID}/stage-3-oozie/job.properties" -DcoordStart=`date -u "+%Y-%m-%dT%H:00Z"` -DuserName=${uID} -DstageThreeOwner=${stage1UserID} -DstageThreeDataPartition=${dataPartID} -DtdServer=${tdServer} -DtdUserID=${tdUserID} -DtdUserIDPassword=${tdUserIDPassword} -Dentity=${entity} -submit)
